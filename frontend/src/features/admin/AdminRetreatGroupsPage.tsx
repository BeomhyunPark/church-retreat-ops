...
function RetreatGroupBoard({ groups, onChanged }: { groups: RetreatGroup[]; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => {
    if (!hasChanges) setMemberOrder(persistedMemberOrder);
  }, [hasChanges, persistedMemberOrder]);
...
function GroupBoard({
  group, activeRegs, draft, width, leaderId, orderedParticipantIds, onDrop, onMemberDrop,
...
function BoardMemberRow({
  participant, isLeader, onToggleLeader, onDropMember
}: {
...
function CandidateSection({
  registrations, totalCount, sortedGroups, getAssignedGroupId, onUnassign
}: {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.7rem", fontWeight: 600, color: assignedGroup ? "var(--color-primary-dark)" : "var(--color-muted)" }}>
        <span>
          {participant.churchCellDepartment}
        </span>
