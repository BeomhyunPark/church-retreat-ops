...
Flyway V17은 기존 데이터를 보존하기 위해 현재 수련회 한 건을 `OPEN` 상태로 생성합니다. 기존 신청, 수련회 조, 공지, 일정은 모두 해당 수련회에 귀속됩니다. V20은 기존 `OPEN` 수련회의 신규 신청 접수를 열린 상태로 이관합니다. 기존 Flyway 파일은 수정하지 않습니다.
